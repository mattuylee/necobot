...
from necobot.config.schema import AgentDefaults
from necobot.agent.tools.base import Tool
from necobot.agent.tools.registry import ToolRegistry
from necobot.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with patch("necobot.agent.loop.ContextBuilder"), \
         patch("necobot.agent.loop.SessionManager"), \
         patch("necobot.agent.loop.SubagentManager") as MockSubMgr:
...
async def test_runner_preserves_reasoning_fields_and_tool_results():
    from necobot.agent.runner import AgentRunSpec, AgentRunner
...
async def test_runner_calls_hooks_in_order():
    from necobot.agent.hook import AgentHook, AgentHookContext
    from necobot.agent.runner import AgentRunSpec, AgentRunner
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".necobot" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from necobot.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".necobot" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "necobot.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "necobot.utils.helpers.logger.warning",
...
async def test_runner_uses_specific_message_after_empty_finalization_retry():
    ...
    from necobot.agent.runner import AgentRunSpec, AgentRunner
    from necobot.utils.runtime import EMPTY_FINAL_RESPONSE_MESSAGE
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("necobot.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "necobot.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("necobot.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from necobot.agent.runner import (
        AgentRunSpec,
        AgentRunner,
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from necobot.agent.loop import AgentLoop
    from necobot.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from necobot.bus.events import InboundMessage
    from necobot.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("necobot.agent.tools.filesystem.ListDirTool.execute", fake_execute)
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from necobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_LENGTH_RECOVERIES
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from necobot.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
def test_drop_orphan_tool_results_removes_unmatched_tool_messages():
    from necobot.agent.runner import AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from necobot.agent.loop import AgentLoop
    from necobot.agent.runner import _BACKFILL_CONTENT
    from necobot.bus.events import InboundMessage
    from necobot.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from necobot.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from necobot.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from necobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTIONS_PER_TURN
    from necobot.bus.events import InboundMessage
...
async def test_injection_cycles_capped_at_max():
    ...
    from necobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTION_CYCLES
    from necobot.bus.events import InboundMessage
...
async def test_followup_routed_to_pending_queue(tmp_path):
    ...
    from necobot.agent.loop import UNIFIED_SESSION_KEY
    from necobot.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from necobot.bus.events import InboundMessage
    from necobot.bus.queue import MessageBus
    from necobot.agent.runner import _MAX_INJECTIONS_PER_TURN
...
async def test_drain_injections_on_empty_final_response():
    ...
    from necobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_EMPTY_RETRIES
    from necobot.bus.events import InboundMessage
...
async def test_drain_injections_set_flag_when_followup_arrives_after_last_iteration():
    ...
    from necobot.agent.hook import AgentHook
    from necobot.agent.runner import AgentRunSpec, AgentRunner
    from necobot.bus.events import InboundMessage
