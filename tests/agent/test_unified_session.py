...
from necobot.command.builtin import cmd_new, register_builtin_commands
from necobot.command.router import CommandContext, CommandRouter
from necobot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("necobot.agent.loop.SessionManager"), \
         patch("necobot.agent.loop.SubagentManager") as MockSubMgr, \
         patch("necobot.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates necobot onboard)."""
        from necobot.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from necobot.agent.memory import Consolidator, MemoryStore
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from necobot.agent.loop import UNIFIED_SESSION_KEY
        from necobot.command.builtin import cmd_stop
