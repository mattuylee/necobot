...
from necobot.channels.matrix import _build_matrix_text_content

import necobot.channels.matrix as matrix_module
from necobot.bus.events import OutboundMessage
from necobot.bus.queue import MessageBus
from necobot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from necobot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("necobot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "necobot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("necobot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "necobot.channels.matrix.asyncio.create_task", _fake_create_task
    )
