...
from necobot.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    ...
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="necobot", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("necobot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import necobot.agent.tools.web as web_mod
