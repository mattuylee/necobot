"""Tests for the Necobot programmatic facade."""
...
from necobot.necobot import Necobot, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Necobot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Necobot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("necobot.config.loader.load_config") as mock_load, \
         patch("necobot.necobot._make_provider") as mock_prov:
...
def test_workspace_override(tmp_path):
    ...
    bot = Necobot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from necobot.config.schema import Config
    from necobot.necobot import _make_provider
...
def test_import_from_top_level():
    from necobot import Necobot as N, RunResult as R
    assert N is Necobot
