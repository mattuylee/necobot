...
Build a custom necobot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of necobot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

necobot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `necobot gateway` starts, it scans:

1. Built-in channels in `necobot/channels/`
2. External packages registered under the `necobot.channels` entry point group
...
```
necobot-channel-webhook/
├── necobot_channel_webhook/
...
```python
# necobot_channel_webhook/__init__.py
from necobot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# necobot_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "necobot-channel-webhook"
version = "0.1.0"
dependencies = ["necobot", "aiohttp"]

[project.entry-points."necobot.channels"]
webhook = "necobot_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
necobot plugins list      # verify "Webhook" shows as "plugin"
necobot onboard           # auto-adds default config for detected plugins
```

Edit `~/.necobot/config.json`:
...
```bash
necobot channels login <channel_name>
necobot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `necobot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `necobot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `necobot.config.schema.Base`:
...
Override `default_config()` so `necobot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `necobot-channel-{name}` | `necobot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `necobot_channel_{name}` | `necobot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/necobot-channel-webhook
cd necobot-channel-webhook
pip install -e .
necobot plugins list    # should show "Webhook" as "plugin"
necobot gateway         # test end-to-end
```

## Verify

```bash
$ necobot plugins list
