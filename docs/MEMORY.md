...
necobot's memory is built on a simple belief: memory should feel alive, but it should not feel chaotic.

Good memory is not a pile of notes. It is a quiet system of attention. It notices what is worth keeping, lets go of what no longer needs the spotlight, and turns lived experience into something calm, durable, and useful.

That is the shape of memory in necobot.

## The Design

necobot does not treat memory as one giant file.
...
Memory moves through necobot in two stages.

### Stage 1: Consolidator

When a conversation grows large enough to pressure the context window, necobot does not try to carry every old message forever.
...
This is why necobot's memory is not just archival. It is interpretive.
...
- `SOUL.md` remembers how necobot should sound.
...
After Dream changes long-term memory files, necobot can record that change with `GitStore`.
...
- Older source-based configs may still contain `dream.cron`. necobot continues to honor it for backward compatibility, but new configs should use `intervalH`.
- Older source-based configs may still contain `dream.model`. necobot continues to honor it for backward compatibility, but new configs should use `modelOverride`.
