...
from necobot.config.paths import get_media_dir, get_runtime_subdir
from necobot.config.schema import Base
from necobot.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.necobot/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'necobot channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"necobot-{uuid.uuid4().hex[:12]}"
