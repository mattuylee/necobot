...
If you discover a security vulnerability in necobot, please report it by:
...
chmod 600 ~/.necobot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.necobot/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.necobot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run necobot with a dedicated user account
...
- Keep authentication data in `~/.necobot/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade necobot-ai
```
...
- Subscribe to security advisories for necobot and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash necobot
   sudo -u necobot necobot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.necobot
   chmod 600 ~/.necobot/config.json
   chmod 700 ~/.necobot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.necobot/logs/necobot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.necobot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.necobot/logs/necobot.log
   ```
